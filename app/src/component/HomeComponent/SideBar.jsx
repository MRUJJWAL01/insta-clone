...
const Sidebar = () => {
  ...
  const handleNavClick = (label) => {
    // Create should only open the modal — do NOT change collapse/search/notification state
    if (label === "Create") {
      setActiveMenu("create");
      return;
    }

    // Toggle Search / Notifications only when their items are clicked
    if (label === "Search") {
      setIsSearchOpen((prev) => !prev);
      // when search opens, we want collapsed view (for desktop behaviour in your original code)
      if (!isSearchOpen) setIsCollapsed(true);
      return;
    }

    if (label === "Notifications") {
      setIsNotificationOpen((prev) => !prev);
      if (!isNotificationOpen) setIsCollapsed(true);
      return;
    }
    ...
    // default: navigate to route and reset menus (for other nav items)
    setIsSearchOpen(false);
    setIsNotificationOpen(false);

    // If viewport is wide (>= breakpoint) we keep expanded by default when navigating
  ...
  return (
    <div className="min-h-screen flex ">
      ...
      <Notifications
        ...
        onClose={() => {
          setIsNotificationOpen(false);
          // restore collapse based on viewport when closing
          setIsCollapsed(window.innerWidth < COLLAPSE_BREAKPOINT);
        }}
      />
    </div>
